function getImages(div){
	...
	jQuery(divImage).find("img").each(function(){
		if(count == 1){
			jQuery(this).addClass("select").css({zIndex:zIndexImage});
		}
		else{
			jQuery(this).addClass("desSelect").css({zIndex:zIndexImage,opacity:0});
		}
		count++;
		zIndexImage--;
	});
	...
	function nextImage(){
		...
		if(leftCounter >= contadoraLeft || identifyFinal == true){
			jQuery(divImage).find(".desSelect").each(function(){
				jQuery(this).animate({opacity:0,zIndex:90},100);
			});
			jQuery(divImage).find(".desSelect").first().animate({opacity:1,zIndex:99},1000);
			jQuery(divImage).find(".select").removeClass("select").addClass("desSelect").css({opacity:0,zIndex:90});
		...
		jQuery(divImage).find(".select").first().animate({opacity:0,zIndex:90},1000);
			jQuery(divImage).find(".select").removeClass("select").addClass("desSelect").next().removeClass("desSelect").addClass("select").animate({opacity:1,zIndex:99},"slow");
	...
	if(!jQuery(divImage).find(".select").prev().is("img")){
		...
		jQuery(divImage).find(".select").removeClass("select").addClass("desSelect").animate({opacity:0,zIndex:90},"slow");
		jQuery(divImage).find(".desSelect").last().removeClass("desSelect").addClass("select").animate({opacity:1,zIndex:99},"slow");
		
		jQuery(divImage).find(".desSelect").each(function(){
			jQuery(this).animate({opacity:0,zIndex:90},1500);
		});
		...
		jQuery(divImage).find(".select").prev().animate({opacity:1,zIndex:99},"slow");
		jQuery(divImage).find(".select").removeClass("select").addClass("desSelect").animate({opacity:0,zIndex:90},"slow").prev().removeClass("desSelect").addClass("select");
	...
	if(navigator.userAgent.toLowerCase().indexOf('msie') != -1){
		jQuery("#controlRight").css({opacity:0.5});
		jQuery("#controlLeft").css({opacity:0.5});
	}
